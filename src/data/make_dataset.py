...
data_merged_resampled = data_merged[:1000].resample("200ms").apply(sampling)

#split by the day
days = [g for n, g in data_merged.groupby(pd.Grouper(freq="D"))]
data_merged_resampled =pd.concat([df.resample(rule="200ms").apply(sampling).dropna() for df in days])

data_merged_resampled ["set"] = data_merged_resampled["set"].astype(int)

data_merged_resampled.info()

# --------------------------------------------------------------
# Export dataset
# --------------------------------------------------------------
data_merged_resampled.to_pickle("../../data/interim/01_data_processed.pkl")